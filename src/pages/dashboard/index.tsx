      ...
      render: (rank: number) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {icon}
            <span style={{ marginLeft: 4, fontWeight: 'bold', color }}>
              {rank}
            </span>
          </div>
        );
      ...
      render: (count: number, record: any) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
            ...
            <span style={{ marginLeft: 4, fontSize: '12px', color: '#8C8C8C' }}>
              次
            </span>
          </div>
        );
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              prefix={<UserOutlined />}
              suffix={
                <span className={summary?.userNewYesterday && summary.userNewYesterday > 0 ? styles.positive : styles.negative}>
                  {summary?.userNewYesterday || 0} 
                  {summary?.userNewYesterday && summary.userNewYesterday > 0 ? <ArrowUpOutlined /> : <ArrowDownOutlined />}
                </span>
              }
            />
          </Card>
          ...
          <Card 
            ...
            title={
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <span>热门标签排行榜</span>
                <Tag color="red" style={{ marginLeft: 12 }}>Top 10</Tag>
              </div>
            }
            extra={
                ...
                <Row gutter={16}>
                  <Col span={6}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <span>总使用次数: {topTags.list.reduce((sum, item) => sum + item.count, 0).toLocaleString()}</span>
                    </div>
                  </Col>
                  <Col span={6}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <Tag color="blue" style={{ marginRight: 4 }}>标签总数</Tag>
                      <span>{topTags.list.length}</span>
                    </div>
                  </Col>
                  <Col span={6}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <Tag color="gold" style={{ marginRight: 4 }}>冠军标签</Tag>
                      <span>{topTags.list[0]?.name || '-'}</span>
                    </div>
                  </Col>
                  <Col span={6}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <Tag color="purple" style={{ marginRight: 4 }}>平均使用</Tag>
                      <span>
                        {Math.round(topTags.list.reduce((sum, item) => sum + item.count, 0) / topTags.list.length)}
                      </span>
                    </div>
                  </Col>
                </Row>
